...
type ApiUpsertBillingPeriodRequest struct {
	ctx context.Context
	ApiService *PlansApiService
	key string
...
func (r ApiUpsertBillingPeriodRequest) UpsertBillingPeriodRequestBody(upsertBillingPeriodRequestBody UpsertBillingPeriodRequestBody) ApiUpsertBillingPeriodRequest {
	r.upsertBillingPeriodRequestBody = &upsertBillingPeriodRequestBody
	return r
}

// If the request is made using an API key that is not environment-scoped, specify the environment using this header
func (r ApiUpsertBillingPeriodRequest) XSchematicEnvironmentId(xSchematicEnvironmentId string) ApiUpsertBillingPeriodRequest {
	r.xSchematicEnvironmentId = &xSchematicEnvironmentId
	return r
}

func (r ApiUpsertBillingPeriodRequest) Execute() (*UpsertBillingPeriodResponse, *http.Response, error) {
	return r.ApiService.UpsertBillingPeriodExecute(r)
}

/*
UpsertBillingPeriod Upsert billing period

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param key key
 @return ApiUpsertBillingPeriodRequest
*/
func (a *PlansApiService) UpsertBillingPeriod(ctx context.Context, key string) ApiUpsertBillingPeriodRequest {
	return ApiUpsertBillingPeriodRequest{
		ApiService: a,
		ctx: ctx,
		key: key,
...
func (a *PlansApiService) UpsertBillingPeriodExecute(r ApiUpsertBillingPeriodRequest) (*UpsertBillingPeriodResponse, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *UpsertBillingPeriodResponse
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "PlansApiService.UpsertBillingPeriod")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/billing-periods/{key}/upsert"
	localVarPath = strings.Replace(localVarPath, "{"+"key"+"}", url.PathEscape(parameterValueToString(r.key, "key")), -1)

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.upsertBillingPeriodRequestBody == nil {
		return localVarReturnValue, nil, reportError("upsertBillingPeriodRequestBody is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.upsertBillingPeriodRequestBody
	if r.ctx != nil {
		// API Key Authentication
		if auth, ok := r.ctx.Value(ContextAPIKeys).(map[string]APIKey); ok {
			if apiKey, ok := auth["ApiKeyAuth"]; ok {
