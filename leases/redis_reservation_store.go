...
// inside a multi-key script. Add pairs its two writes to the reservation hash in
// a transaction so a hold cannot land without the expiry that reaps it, which is
// still one key and so still cluster-safe.
...
//
// The hash and its expiry go out as one MULTI/EXEC. Written separately, a crash
// in the gap leaves a reservation row with no TTL: once the sweeper drops its
// index entry nothing points at the row and nothing reaps it, so it sits in
// Redis for good. Both commands address the one key, so a cluster takes the
// transaction too.
func (s *RedisReservationStore) Add(ctx context.Context, reservation ReservationRecord) error {
	...
	hold := s.client.TxPipeline()
	hold.HSet(ctx, hashKey, map[string]any{
		"id":               reservation.ID,
	...
	hold.PExpireAt(ctx, hashKey, fromEpochMs(expiresMs+reservationTTLGraceMs))
	if _, err := hold.Exec(ctx); err != nil {
		return err
	}

	// The two indexes (the expiry sorted set the sweeper pops from, the
	// per-tenant hash ReservedCredits sums) only depend on the hash existing, so
	// they go out together: one round-trip wave rather than two. They stay
	// outside the transaction because their keys hash to other slots, and a
	// cluster refuses a transaction that spans slots. A failure here at worst
	// leaves an un-indexed reservation that the TTL reaps, its slice reclaimed
	// when the lease expires, never a double-spend. This sits on every allowed
	// check, so the latency matters.
	indexes := s.client.Pipeline()
	indexes.ZAdd(ctx, s.indexKey(), redis.Z{
